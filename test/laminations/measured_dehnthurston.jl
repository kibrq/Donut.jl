...
@testset "Dehn-Thurston train track from coordinates" begin
    ...
    tt, encoding = measured_dehnthurstontrack(pd, [(1, -3), (4, -4), (3, 10)])

    pd = PantsDecomposition([(1, 2, 3), (-3, -2, -1)])
    tt, encoding = measured_dehnthurstontrack(pd, [(5, 5), (4, 0), (3, -99)])

    pd = PantsDecomposition([(3, 2, 1), (-1, 4, 5)])
    tt, encoding = measured_dehnthurstontrack(pd, [(BigInt(4), BigInt(-3))])
    # @test extremal_branch(tt, 1) == -extremal_branch(tt, -1, 1)
