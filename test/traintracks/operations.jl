...
using Donut: _find_new_switch_number!, _find_new_branch_number!

@testset "Renaming branches" begin
    tt = PlainTrainTrack([[1, 3], [-1, -3]])
    @test !isbranch(tt, 2)
...
@testset "twisted" begin
    tt = PlainTrainTrack([[1, 2, 3, 4, 5], [-5, -4, -3, -2, -1]], [3, 5])
...
@testset "Collapsing branches" begin
    ...
    tt = PlainTrainTrack([[1, 2], [-3], [3], [-1, -2]], [1, 3])
...
@testset "Adding a switch on a branch" begin
    ...
    tt = TrainTrack([[1, 2], [-1, -2]], twisted_branches=[1])
    @test apply_tt_operation!(tt, AddSwitchOnBranch(1)) == 2
    @test istwisted(tt, 1)
    @test !istwisted(tt, 3)
end
...
@testset "Peeling" begin
    ...
    tt = PlainTrainTrack([[1, 2], [-3], [3], [-1, -2]], [3])
...
@testset "Folding" begin
    ...
    tt = PlainTrainTrack([[1, 2], [-3], [3, 4], [-4, -1, -2]], [3])
