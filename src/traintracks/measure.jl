export TrainTrackMeasure, branchmeasure, zeromeasure


struct TrainTrackMeasure{T<:Number}
    # values::Array{T, 1}
    values::Dict{Int, T}

    function TrainTrackMeasure{T}(tt::TrainTrack, valuedict::Dict{Int, T}, allownegatives::Bool=false) where {T<:Number}
        if !allownegatives
    ...
    function TrainTrackMeasure{T}(tt::TrainTrack, valuearray::Array{T, 1}) where {T<:Number}
        ...
        TrainTrackMeasure{T}(tt, valuedict)
    end 
end
...
function zeromeasure(tt::TrainTrack, type)
    ...
    TrainTrackMeasure{type}(tt, zeros(type, length(branches(tt))))
end


function branchmeasure(measure::TrainTrackMeasure, branchindex::Int)
    try 
