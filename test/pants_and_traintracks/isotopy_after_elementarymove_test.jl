...
@testset "Dehn twists" begin
    ...
    tt, encodings = dehnthurstontrack(pd, [1, 0], [LEFT, RIGHT, LEFT])
    update_encodings_aftermove!(tt, pd, Twist(-3, LEFT), encodings)
    @test gluinglist(pd) == [(1, 2, 3), (-3, -2, -1)]
    # @test sum(length(item[2]) for item in encoding_changes) == 4   

    pd = PantsDecomposition([(1, 2, 3), (-3, -2, -1)])
    tt, encodings = dehnthurstontrack(pd, [1, 0], [LEFT, RIGHT, LEFT])
    update_encodings_aftermove!(tt, pd, Twist(1, RIGHT), encodings)
    @test gluinglist(pd) == [(1, 2, 3), (-3, -2, -1)]
...
@testset "Half twists" begin
    pd = PantsDecomposition([(1, 2, 3), (-3, -2, -1)])
    tt, encodings = dehnthurstontrack(pd, [1, 0], [LEFT, RIGHT, LEFT])
    update_encodings_aftermove!(tt, pd, HalfTwist(-1, LEFT), encodings)
...
@testset "First move" begin
    ...
    tt, encodings = dehnthurstontrack(pd, [3, 0], [LEFT, RIGHT, LEFT])
    update_encodings_aftermove!(tt, pd, FirstMove(1), encodings)
    @test gluinglist(pd) == [(1, -1, 2), (-2, 3, -3)]
    # @test sum(length(item[2]) for item in encoding_changes) == 7

    pd = PantsDecomposition([(1, -1, 2), (-2, 3, -3)])
    tt, encodings = dehnthurstontrack(pd, [3, 0], [LEFT, RIGHT, LEFT])
    update_encodings_aftermove!(tt, pd, FirstMove(3), encodings)
    @test gluinglist(pd) == [(1, -1, 2), (-2, 3, -3)]
...
@testset "Second move" begin
    pd = PantsDecomposition([(1, 2, 3), (-3, -2, -1)])
    tt, encodings = dehnthurstontrack(pd, [1, 0], [LEFT, RIGHT, LEFT])
    update_encodings_aftermove!(tt, pd, SecondMove(1), encodings)
