...
@testset "Peel-fold Dehn twist" begin
    ...
    apply_change_of_marking_to_tt!(ttnet, 1, pd, Twist(2), longencodings)
    ...
    ttnet = TrainTrackNet([tt])
    apply_change_of_marking_to_tt!(ttnet, 1, pd, Twist(2, LEFT), longencodings)
...
@testset "Peel-fold second move 1" begin
    ...
    ttnet = TrainTrackNet([tt])
    apply_change_of_marking_to_tt!(ttnet, 1, pd, SecondMove(2), longencodings)
...
@testset "Peel-fold second move 3" begin
    ...
    ttnet = TrainTrackNet([tt])
    @test sort(tt.measure.values[1:numbranches(tt)]) == [1, 1, 2, 2, 3, 6, 6, 11, 20]
    apply_change_of_marking_to_tt!(ttnet, 1, pd, SecondMove(3), longencodings)
...
@testset "Peel-fold first move 1" begin
    ...
    tt, pd, longencodings, orderedmeasures = separating_tt_large_central_intersection()
    ttnet = TrainTrackNet([tt])
    apply_change_of_marking_to_tt!(ttnet, 1, pd, FirstMove(1), longencodings)
    @test sort(tt.measure.values[1:numbranches(tt)]) == [1, 1, 4, 6, 6, 8, 9, 9, 14]
    apply_change_of_marking_to_tt!(ttnet, 1, pd, FirstMove(-3), longencodings)
    @test sort(tt.measure.values[1:numbranches(tt)]) == [1, 1, 6, 8, 9, 9, 10, 10, 18]
    apply_change_of_marking_to_tt!(ttnet, 1, pd, FirstMove(-3, true), longencodings)
    @test sort(tt.measure.values[1:numbranches(tt)]) == [1, 1, 4, 6, 6, 8, 9, 9, 14]
    apply_change_of_marking_to_tt!(ttnet, 1, pd, FirstMove(1, true), longencodings)
    @test sort(tt.measure.values[1:numbranches(tt)]) == orderedmeasures
end
